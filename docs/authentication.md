...
**Android uses a non-standard WebAuthn origin.** Browsers use `https://example.com`; Android uses `android:apk-key-hash:<base64-sha256>`. The upstream `firebase-web-authn` extension doesn't support this — the patch adds the `ALLOWED_ORIGINS` parameter to fix it.
